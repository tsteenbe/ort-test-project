...
[#--
    ...
    3. A dependencies section, introduced by "This software depends on external packages and source
       code.", with one subsection per (non-excluded) package. Each package subsection contains:
         * a "Package <namespace>:<name>:<version>" heading, its "PURL:" (without any qualifiers or subpath, i.e.
           truncated at the first '?' or '#') and its effective "License:" expression
...
License: ${packageLicense}
    ...
    [#--
        ...
        "License choices:" so the block stays readable. Each choice renders as "<given>; License used: <choice>", or just
...
License choices:
  ...
  [#if licenseChoice.given??]${licenseChoice.given}; License used: ${licenseChoice.choice}[#else]${licenseChoice.choice}[/#if]
            [/#list]
        [#else]
            [#assign licenseChoice = appliedLicenseChoices?first]
License choice: [#if licenseChoice.given??]${licenseChoice.given}; License used: ${licenseChoice.choice}[#else]${licenseChoice.choice}[/#if]
        [/#if]
    [/#if]
[/#macro]
[#list packages?filter(p -> !p.excluded) as package]
    ...
    [#--
        Strip the qualifiers and the subpath from the PURL, i.e. everything starting at the first '?' or '#'.
    --]
    [#assign packagePurl = helper.getPackage(package.id).purl?keep_before("?")?keep_before("#")]
...
[#if !(ortResult.hasLabel("sw_type", "Car", false) || ortResult.hasLabel("sw_type", "Mobile", false)) ]
In case of individualized licenses, please refer to repositories according to the SBOM.
[/#if]
